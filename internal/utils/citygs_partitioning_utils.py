...
class CityGSPartitioning(Partitioning):
    ...
    def align_bounding_box(cls, bounding_box: MinMaxBoundingBox, origin: torch.Tensor, block_dim: list, contract: bool):
        ...
        assert torch.all(origin >= bounding_box.min), "center {} out-of-min-bound {}. try --auto_origin".format(origin, bounding_box.min)
        assert torch.all(origin <= bounding_box.max), "center {} out-of-max-bound {}. try --auto_origin".format(origin, bounding_box.max)
